...
def parseargs():
    ...
    parser.add_argument('--desc_file', type=str, default='description.csv')
...
def set_up(args):
    ...
    SAVE_CODE_LIST = ["arch/vlm.py", "dataset_vlm.py", "utils/ops.py", "utils/utils.py", "main_task3.py",
                      'job/train_task3.job']
    TIME = utils.get_time()
...
def main(args):
    ...
    model = VLM(model_url_vision=args.model_url_vision, model_url_language=args.model_url_lang, n_classes=1,
                ...
                weight_decay=args.weight_decay, criterion=ops.get_criterion(args.loss, 1),
                steps_per_epoch=steps_per_epoch, save_dir=model_dir, with_lora=args.lora, lora_rank=args.lora_rank)
